...
import jmemorize.gui.swing.panels.CardPanel.CardPanelObserver;
import jmemorize.gui.swing.panels.CardSidePanel.Mode;
...
//        addCardSide("Notes", new TextCardSidePanel());
    ...
    public void setHtml(String frontSide, String backSide) {
        DocumentListener docListener = new DocumentListener()
        {
            public void changedUpdate(DocumentEvent e)
            {
    	...
    	m_frontSide.setHtml(frontSide).addDocumentListener(docListener);
    	m_backSide.setHtml(backSide).addDocumentListener(docListener);
    }

    protected void notifyHtmlObservers()
    {
    	try {
    		m_frontSide.updateBrowser();
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    	try {
    		m_backSide.updateBrowser();
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    	super.notifyHtmlObservers();
    }
    
    public String getFrontHtml() {
    	if (m_frontSide != null) {
        	return m_frontSide.getHtml();
    	} else {
    		return "";
    	}
    }
    ...
    public void setImageMode(Mode mode, boolean editable) {
    	m_frontSide.setImageMode(mode, editable);
    	m_backSide.setImageMode(mode, editable);
	}    
