...
public class CardSidePanel extends JPanel
{
    ...
    public static enum Mode {TEXT, IMAGE, TEXT_AND_IMAGE, WEB};
    ...
    private JFXPanel m_webPanel = new JFXPanel();
    private JTextPane               m_htmlEditPane       = new JTextPane();
    private JScrollPane             m_htmlEditScrollPane = new JScrollPane(m_htmlEditPane);
    ...
    private JButton                 m_imageWebModeButton;
    ...
    public void setEditable(boolean editable)
    {
        ...
        m_htmlEditPane.setEditable(editable);
    }
    ...
    public Document setHtml(String html) {
    	this.m_html = html;
    	try {
    		m_htmlEditPane.setText(html);
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    	try {
            Platform.runLater(() -> {
        		m_webEngine.loadContent(m_html);
            });        
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    	return m_htmlEditPane.getDocument();
    }
    ...
    public void setImageMode(Mode mode, boolean editable)
    {
        ...
        m_imageWebModeButton.setSelected(mode == Mode.WEB);
...
//        textScrollPane.setBorder(null);
        ...
        case WEB:
        	if (editable) {
                m_contentPanel.setLayout(new GridLayout(1, 2));
                m_contentPanel.add(m_htmlEditScrollPane, BorderLayout.CENTER);
                m_contentPanel.add(m_webPanel, BorderLayout.EAST);
                m_htmlEditPane.requestFocus();
        	} else {
                m_contentPanel.setLayout(new BorderLayout());
                m_contentPanel.add(m_webPanel, BorderLayout.CENTER);
        	}
            break;
...
//        
    ...
    private void initComponents()
    {
        ...
        m_htmlEditPane.setBackground(ColorConstants.CARD_PANEL_COLOR);
        
        m_htmlEditScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        m_htmlEditScrollPane.setBorder(null);
        ...
        Platform.runLater(() -> {
            ...
            m_webPanel.setScene(new Scene(m_webView));
            //m_webEngine.load("http://www.stackoverflow.com/");
    		m_webEngine.loadContent(m_html);
        });        
    }
    ...
    private void buildImageBar()
    {
        ...
        m_imageWebModeButton = new JButton(loadIcon("web.png")); //$NON-NLS-1$
        m_imageWebModeButton.addActionListener(new SetImageModeAction(Mode.WEB));
        m_imageBar.add(m_imageWebModeButton);
